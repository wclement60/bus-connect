-- Fonction RPC optimisée pour charger rapidement les arrêts du plan interactif
...
-- Fonction RPC optimisée pour charger rapidement les tracés des lignes
...
RETURNS TABLE (
  ...
  coordinates JSONB  -- Array de [longitude, latitude] pour chaque point du tracé
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  ...
  -- Récupérer les points des shapes ordonnés
  shape_points AS (
    SELECT 
      rs.route_id,
      s.shape_pt_sequence,
    ...
    ORDER BY rs.route_id, s.shape_pt_sequence
  ),
  -- Agréger les points par route
  route_coordinates AS (
    SELECT 
      sp.route_id,
      jsonb_agg(
        jsonb_build_array(sp.shape_pt_lon, sp.shape_pt_lat) 
        ORDER BY sp.shape_pt_sequence
      ) as coordinates
    FROM shape_points sp
    GROUP BY sp.route_id
  )
