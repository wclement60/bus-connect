-- Script d'installation des fonctions de suppression optimisée
-- Exécuter ce script pour installer toutes les fonctions nécessaires à la suppression rapide des réseaux

-- Fonctions auxiliaires pour la gestion des triggers et la vérification des tables
...
CREATE OR REPLACE FUNCTION public.delete_network_data(network_id_param TEXT)
RETURNS JSONB AS $$
BEGIN
  -- Cette fonction appelle simplement la nouvelle fonction optimisée
  RETURN simple_delete_network(network_id_param);
END;
$$ LANGUAGE plpgsql;
...
DO $$
BEGIN
  RAISE NOTICE 'Installation des fonctions de suppression optimisée terminée avec succès.';
  RAISE NOTICE 'Utilisez la fonction simple_delete_network(network_id) pour supprimer rapidement un réseau.';
END $$; 